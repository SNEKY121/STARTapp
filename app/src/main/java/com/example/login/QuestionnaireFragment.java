...
public class QuestionnaireFragment extends Fragment {
    ...
    private void setProgress() {
        try {
            if (connect != null) {
                if (capitol_id == CoursesFragment.getCapitol()) {
                    int progress = (int) ((float) CoursesFragment.getCapitol() / (float) StartCourseFragment.getNrCapitole() * (float) (questionNumber) / (float) (numberOfQuestions) * 100 / StartCourseFragment.getNrCapitole());
